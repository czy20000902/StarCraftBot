...
BT_DECO_REPEATER::BT_DECO_REPEATER(std::string name, BT_NODE* parent, size_t nMaxRepeats, bool repeatForever, bool endOnFailure)
    : NMaxRepeats(std::min(nMaxRepeats,(size_t)1)),RepeatForever(repeatForever),EndOnFailure(endOnFailure),  BT_DECORATOR(name,parent) 
{
	NRepeats =0;
}
